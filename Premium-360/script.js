...
document.getElementById('nuevo-empleado-form').addEventListener('submit', function(e) {
  e.preventDefault();
  ...
  const nuevoEmpleado = {
    ...
    evaluacion: {
      ...
      comunicacion: parseFloat(document.getElementById('comunicacion-ad').value)
    }
  };
...
function filtrarProblemasCultural(empleados) {
  return empleados.filter(empleado => {
    return empleado.evaluacion.equipo < 2.5 || 
           empleado.evaluacion.comunicacion < 2.5;
  });
}
// Para que sirvaa
const empleados = JSON.parse(localStorage.getItem('empleados')) || [];
...
function clasificarEmpleado(empleado) {
  ...
  if (resultados >= 15 && evaluacion.equipo <= 2.5) {
    ...
    return {
      ...
      razon: "Alto desempeño y excelente integraciona los valores de la empresa"
    };
  } else {
    return {
...
function evaluarEquipo() {
  ...
  resultadosDiv.innerHTML = empleados.map(empleado => `
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!evaluationForm) {
    console.error('Error: No se encontró el formulario con ID "evaluation-form"');
    return;
  }
...
function mostrarResultadoIndividual(empleado) {
  ...
  resultadosDiv.innerHTML = `
    <div class="resultado-individual">
      ...
      <p><strong>Participación de mercado:</strong> ${evaluacionCompleta.resultados}%</p>
      <p><strong>Trabajo en equipo:</strong> ${evaluacionCompleta.evaluacion.equipo}/5</p>
      <p><strong>Comunicación:</strong> ${evaluacionCompleta.evaluacion.comunicacion}/5</p>
      <p class="decision ${evaluacionCompleta.decision.includes('NO') ? 'decision-riesgo' : 'decision-ok'}">
...
function actualizarListaEmpleados() {
  ...
  lista.innerHTML = evaluaciones.map(emp => `
    <div class="empleado-lista-item">
      <h4>${emp.nombre}</h4>
      <p>${emp.puesto} | ${emp.resultados}% mercado</p>
      <p class="decision-mini ${emp.decision.includes('NO') ? 'decision-riesgo' : 'decision-ok'}">
        ${emp.decision}
      </p>
    </div>
  `).join('');
...
document.getElementById('btn-evaluar-equipo').addEventListener('click', function() {
  const empleados = JSON.parse(localStorage.getItem('empleados')) || [];
  const contenedorScroll = document.querySelector('.contenedor-scroll-equipo');
  const resultadosDiv = document.getElementById('resultados-equipo');

  // 1. Generar HTML de resultados
  resultadosDiv.innerHTML = empleados.map(emp => {
    const decision = clasificarEmpleado(emp);
    return `
      <div class="resultado-equipo-item">
        <h4>${emp.nombre}</h4>
        <p>${emp.puesto} | ${emp.resultados}%</p>
        <p class="decision-mini ${decision.decision.includes('NO') ? 'decision-riesgo' : 'decision-ok'}">
          ${decision.decision}
        </p>
      </div>
    `;
...
function filtrarEmpleados() {
  ...
  switch(criterio) {
    case 'riesgo':
      ...
      break;
    case 'promocion':
      empleadosFiltrados = empleados.filter(e => e.evaluacion.equipo >= 4 && e.resultados >= 12);
      break;
    default:
  ...
  contenido.innerHTML = empleadosFiltrados.map(emp => `
    <div class="empleado-item">
      <h4>${emp.nombre}</h4>
      <p>Equipo: ${emp.evaluacion.equipo}/5 | Resultados: ${emp.resultados}%</p>
      <p class="decision ${emp.evaluacion.equipo <= 2.5 ? 'decision-riesgo' : 'decision-ok'}">
...
function cargarListaEmpleados() {
  const empleados = JSON.parse(localStorage.getItem('empleados')) || [];
  const contenedor = document.querySelector('#lista-empleados .contenedor-scroll');
...
function mostrarResultados(data, origen = 'test') {
  ...
  const resultadoHTML = `
    <div class="resultado ${origen}">
      <h3>${data.nombre}</h3>
      <p><strong>Origen:</strong> ${origen === 'test' ? 'Evaluación por Test' : 'Registro Rápido'}</p>
      <table>
...
document.getElementById('evaluation-form').addEventListener('submit', function(e) {
  ...
  if (radios.length < 6) { // 6 preguntas en total
...
function actualizarListaEmpleados() {
  ...
  lista.innerHTML = evaluaciones.map(emp => `
    <div class="empleado-item">
      <h4>${emp.nombre}</h4>
      <p>Puesto: ${emp.puesto}</p>
      <p class="decision ${emp.decision.includes('NO') ? 'decision-riesgo' : 'decision-ok'}">
        ${emp.decision}
      </p>
